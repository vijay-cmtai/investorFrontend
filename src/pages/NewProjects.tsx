...
import { Search, Layers, X, ListFilter } from "lucide-react";
...
// Aapka original filter state interface
...
const NewProjects = () => {
  ...
  // Aapka original Redux and state logic (No changes)
  ...
  // Aapka original useEffect logic (No changes)
  useEffect(() => {
  ...
  // Aapka original useMemo logic for filtering (No changes)
  ...
  // Aapka original useMemo logic for available cities (No changes)
  ...
  // Aapka original useMemo logic for property types (ERROR FIXED)
  const availablePropTypes = useMemo(() => {
    if (!Array.isArray(properties)) return [];
    const propTypes = properties
      .map((p: Property) => p.property_type)
      .filter((type): type is string => Boolean(type)); // FIXED: yahan "city is string" tha
    return [...new Set(propTypes)];
  }, [properties]);

  // Aapke original event handlers (No changes)
      ...
      >
        <div className="container mx-auto px-4 text-center animate-fade-in">
          ...
          <p className="text-lg md:text-xl text-neutral-200 max-w-3xl mx-auto mb-8">
            Invest in tomorrow's landmarks with our curated list of new
            projects.
          </p>
          <Card className="max-w-4xl mx-auto bg-background/90 backdrop-blur-sm p-4 shadow-xl">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
              <div className="md:col-span-2">
                ...
                <Input
                  id="hero-search"
                  placeholder="e.g. 'Emaar', 'Gurgaon', 'Luxury Apartments'"
              ...
              <Button size="lg" className="w-full">
                <Search className="w-5 h-5 mr-2" /> Search Projects
              </Button>
            </div>
          </Card>
        </div>
            ...
            <main className="lg:col-span-3">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-bold text-foreground">
                  {isLoading
                    ? "Searching..."
                    : `${filteredProperties.length} New Project${filteredProperties.length !== 1 ? "s" : ""} Found`}
                </h2>
...
// --- Reusable Sub-components for better UI structure ---
const FiltersSidebar = ({
  ...
  <Card className="shadow-lg">
    ...
    <CardContent className="space-y-4">
      ...
      <div>
        <Label htmlFor="filter-bedrooms">Bedrooms (BHK)</Label>
        <Select
  ...
  <div className="text-center py-16 col-span-full animate-fade-in">
    <div className="max-w-md mx-auto">
      ...
      <h3 className="text-2xl font-semibold text-foreground mb-3">
        No New Projects Match Your Criteria
      </h3>
      <p className="text-muted-foreground mb-8">
        Try adjusting your filters or check back soon for the latest launches.
      </p>
